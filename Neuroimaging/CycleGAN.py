...
from tensorflow.keras.models import Model # replaced # keras.engine.topology import Network
import imageio
from PIL import Image # replaced # from scipy.misc import imsave, toimage
import numpy as np
import random
import datetime
import time
...
# UPdate # tf.logging.set_verbosity(tf.logging.ERROR)
...
class CycleGAN():
    ...
    def call(self, x, training=False):
        padding_size = 1  # Example padding size
        x = reflection_padding(x, padding_size)
        # Continue with your model's operations
        return x
    ...
    def create_discriminator_and_generator(self):
        ...
        self.D_A_static = Model(inputs=image_A, outputs=guess_A, name='D_A_static')
        self.D_B_static = Model(inputs=image_B, outputs=guess_B, name='D_B_static')
    ...
    def train(self, train_A_dir, normalization_factor_A, train_B_dir, normalization_factor_B, models_dir, batch_size=10, epochs=200, cycle_loss_type='L1', use_resize_convolution=False, use_supervised_learning=False, output_sample_flag=True, output_sample_dir=None, output_sample_channels=1, dropout_rate=0):
        ...
        if self.train_A.shape[1] <= padding_size * 2 or self.train_A.shape[2] <= padding_size * 2:
            raise ValueError("Training data dimensions are too small for the specified padding size.")
            ...
            for loop_j in range(self.loop_num):
                ...
                if output_sample_flag:
                    if (loop_j + 1) % 5 == 0:
                        ...
                        cmin=-1
                        cmax=1
                        scaled_array = np.clip(output_sample, cmin, cmax)  # Replace cmin, cmax with actual values
                        scaled_array = 255 * (scaled_array - cmin) / (cmax - cmin)
                        scaled_array = scaled_array.astype('uint8')  # Convert to 8-bit integer
                        Image.fromarray(scaled_array).save(output_sample_dir)
            if (epoch_i + 1) % 20 == 0:
    ...
    def reflection_padding(self, x, h_pad, w_pad):
        input_shape = tf.shape(x)
        height, width = input_shape[1], input_shape[2]
        
        # Ensure padding values are valid
        h_pad = min(h_pad, height - 1)
        w_pad = min(w_pad, width - 1)
    ...
    def lse(self, y_true, y_pred):
        loss = tf.reduce_mean(tf.math.square(tf.subtract(y_pred, y_true)))
        return loss
