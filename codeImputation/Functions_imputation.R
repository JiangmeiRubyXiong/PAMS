...
metrics_result_markers <- function(markers, root_dir="~/ImageImputation/metricsAllMarker", palette_def="lisa::JacobLawrence"){
  ...
  for(marker in markers){
    ...
    for(rep.i in 1:500){
      ...
      metric.df$rep <- rep.i
      metric.df$truth.mean <- true.res$means
  ...
  metric.df.all.bias$methods <- factor(metric.df.all.bias$methods, levels=c("bySample", "local", "simple", "mean"),
                                       labels = c("Hierachical Local Bootstrap", "Local Bootstrap", "Boostrap", "Synthesis"))

  p1 <- ggplot(metric.df.all.bias)+
    geom_errorbar(aes(xmin=bias.mean-1.96*bias.se, xmax=bias.mean+1.96*bias.se, y=methods,color=phenotype), width=.2, position=position_dodge(.9)) +
    geom_point(aes(x=bias.mean, y=methods, color=phenotype), alpha=0.7, position=position_dodge(.9))+facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+
    geom_vline(xintercept=0, linetype=3, alpha=0.6)+theme_minimal()+ggtitle(paste0("Mean Bias in 500 reps"))+xlab("Bias")+scale_color_paletteer_d(palette_def)
  ...
  p2 <- ggplot(metric.df.all.bias)+
    ...
    facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+
    geom_vline(xintercept = 1, linetype=3, alpha=0.6)+
    theme_minimal()+ggtitle(paste0("Standard error estimation accuracy"))+scale_fill_paletteer_d(palette_def)
  
  # plot 3: standard error deviations
  #   ggplot(metric.df.all)+
  #   geom_boxplot(aes(sd*stats, methods, fill=phenotype), alpha=0.5, outlier.size=0.5, linewidth = 0.1, outlier.alpha=0.3)+
  #   facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+xlab("")+
  #   theme_minimal()+ggtitle(paste0("ci length/2"))+scale_fill_paletteer_d(palette_def)
  # 
  # ggplot(metric.df.all)+
  #   geom_boxplot(aes(sd, methods, fill=phenotype), alpha=0.5, outlier.size=0.5, linewidth = 0.1, outlier.alpha=0.3)+
  #   facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+xlab("")+
  #   theme_minimal()+ggtitle(paste0("sd"))+scale_fill_paletteer_d(palette_def)
  # 
  # ggplot(metric.df.all)+
  #   geom_boxplot(aes(abs(bias), methods, fill=phenotype), alpha=0.5, outlier.size=0.5, linewidth = 0.1, outlier.alpha=0.3)+
  #   geom_vline(xintercept = 0, linetype=2, alpha=0.6)+
  #   facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+xlab("bias")+
  #   theme_minimal()+ggtitle(paste0("abs(bias)"))+scale_fill_paletteer_d(palette_def)
  # 
  # ggplot(metric.df.all)+
  #   geom_boxplot(aes(sd*stats-abs(bias), methods, fill=phenotype), alpha=0.5, outlier.size=0.5, linewidth = 0.1, outlier.alpha=0.3)+
  #   geom_vline(xintercept = 0, linetype=2, alpha=0.6)+
  #   facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+xlab("bias")+
  #   theme_minimal()+ggtitle(paste0("CI/2-abs(bias)"))+scale_fill_paletteer_d(palette_def)
  # 
  # ggsave("~/ImageImputation/metricsAllMarker/fig2check2.pdf", height = 5, width = 9) 
  ...
  p3 <- ggplot(metric.df.all.bias)+
    ...
    facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+
    geom_vline(xintercept = 1, linetype=3, alpha=0.6)+
    theme_minimal()+ggtitle("Relative MSE (v.s. full data estimator)")+scale_fill_paletteer_d(palette_def)
  ...
  table2 <- reshape2::melt(
    data.frame((table1[,4:7]), quantile=table1$quantile, phenotype=table1$phenotype, marker=table1$marker),
                 id.vars=c("quantile", "phenotype", "marker"))
  
  # table2$label <- round(table2$value+0.95,3)
  
  table2$methods <- table2$variable
  # p4 <- ggplot(table2, aes(quantile, variable, fill= value)) + 
  #   geom_tile()+scale_fill_continuous_divergingx(palette = 'Spectral', mid = 0)+
  #   geom_text(aes(label=label),size=2)+
  #   facet_wrap(~marker+phenotype, nrow=1)+theme_minimal()
  
  p4 <- ggplot(table2)+
    geom_bar(stat="identity",aes(value-0.95, methods, fill=phenotype), alpha=0.7, position=position_dodge(), width=0.5)+
    facet_grid(rows=vars(quantile), cols=vars(marker), scales = "free_x")+
    xlab("Percentage")+ scale_x_continuous(labels = function(x) x + 0.95)+geom_vline(xintercept = 0, linetype=2, alpha=0.8)+
    theme_minimal()+ggtitle("Coverage of 95% CI")+scale_fill_paletteer_d(palette_def)
